        ...
        <div class="p-5 w-9/12 laptop:w-full  mx-auto ">
            <div class=" bg-medium w-full h-250 rounded-3xl shadow-3xl animate__animated animate__fadeInUp">
                ...
                @if ($admin->count() > 0)
                            ...
                            <li
                                ...
                                <div class="flex items-center animate__animated animate__fadeInLeft">
                                    ...
                                    <div class="flex flex-col justify-center ml-3 lg:ml-10 ">
                                        ...
                                        @if($data->devisi)
                                            <h3 class="text-lg tablet:text-sm">{{ $data->devisi->nama_devisi }}</h3>
                                        @else
                                            <p class="text-lg tablet:text-sm text-red-500">Devisi tidak tersedia</p>
                                        @endif
                                    </div>
                                ...
                                {{-- hanya role supes yang bisa melihat data admin --}}
                                @auth
                                    @if (auth()->user()->role === 'super')
                                        <a class="group/edit invisible hover:bg-primary hover:shadow-3xl  group-hover/item:visible w-28 h-10 tablet:w-16 rounded-full flex items-center justify-center tablet:hidden"
                                            href="{{ route('data-admin.show', $data->nip) }}">
                                    ...
                                    @else
                                    <a id="hakAkses_{{ $data->nip }}"
                                        class="group/edit invisible hover:bg-primary hover:shadow-3xl group-hover/item:visible w-28 h-10 tablet:w-16 rounded-full flex items-center justify-center tablet:hidden"
                                        href="#">
                                        <span class="group-hover/edit:text-white  tablet:text-sm">Detail</span>
                                        <iconify-icon icon="mdi:pan-right" class="text-4xl tablet:text-2xl tablet:hidden"></iconify-icon>
                                    </a>
                                    @endif
                                @endauth
                            </li>
                    ...
                    @else
                    <p class="text-2xl mt-10 text-center text-white"> Admin(pegawai) Tidak Di Temukan.</p>
                    @endif
                </div>
            </div>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        @foreach ($admin as $data)
            document.querySelector('#hakAkses_{{ $data->nip }}').addEventListener('click', function(e) {
                e.preventDefault();
    ...
    document.addEventListener('DOMContentLoaded', function () {
            ...
            document.querySelector('#btnAkses').addEventListener('click', function(e) {
                e.preventDefault();
